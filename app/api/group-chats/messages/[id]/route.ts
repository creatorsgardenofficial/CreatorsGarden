...
export async function PUT(
  ...
  try {
    ...
    // メッセージを取得
    const message = await getGroupMessageById(id);
    if (!message) {
      return NextResponse.json(
        { error: 'メッセージが見つかりません' },
        { status: 404 }
    ...
    // メッセージを更新
    const updatedMessage = await updateGroupMessage(id, { content });
    if (!updatedMessage) {
      return NextResponse.json(
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.error('Update group message error occurred');
    }
    return NextResponse.json(
      { error: 'メッセージの更新に失敗しました' },
      { status: 500 }
...
export async function DELETE(
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.error('Delete group message error occurred');
    }
    return NextResponse.json(
      { error: 'メッセージの削除に失敗しました' },
      { status: 500 }
