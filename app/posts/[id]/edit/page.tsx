...
import { PostType, User, Post, PostStatus } from '@/types';
...
export default function EditPostPage() {
  ...
  const fetchPost = async () => {
    try {
      ...
      if (!userData.user || userData.user.id !== postData.userId) {
      ...
      let urls: Array<{ url: string; description: string }> = [];
      
      if (postData.urls && postData.urls.length > 0) {
        // 新しい形式（オブジェクト配列）または古い形式（文字列配列）を判定
        if (typeof postData.urls[0] === 'string') {
          // 古い形式（文字列配列）の場合
          urls = (postData.urls as string[]).map(url => ({ url, description: '' }));
        } else {
          // 新しい形式（オブジェクト配列）の場合
          urls = (postData.urls as Array<{ url: string; description?: string }>).map(item => ({
            url: item.url,
        ...
        urls = [{ url: postData.url || '', description: '' }];
      } else {
        urls = [{ url: '', description: '' }];
      }
      
      // 各要素のurlとdescriptionが確実に文字列になるように保証
      urls = urls.map(item => ({
        url: item.url || '',
        description: item.description || '',
      }));
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-white"
                >
                  <option value="open">Open（メンバーを探している）</option>
                  <option value="closed">Closed（メンバーが見つかった）</option>
                </select>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                  メンバーが見つかっていない場合は「Open」、すでにメンバーが見つかっている場合は「Closed」を選択してください
                </p>
              </div>
